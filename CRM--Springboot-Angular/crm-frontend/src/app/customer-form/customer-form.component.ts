...
export class CustomerFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.id != -1)
      this.service.retrieveCustomer("dibs-demo", this.id).subscribe(
  ...
  onSave() {
      if(this.id == -1) {
        ...
        this.service.addCustomer("dibs-demo", this.customer).subscribe(
          response => {
        ...
        this.service.updateCustomer("dibs-demo", this.customer).subscribe(
          response => {
            console.log(response);
            this.router.navigate(['customers']);
          }
